...
const TASK_ENDPOINT = '/api/projects/{project}/tasks';
...
beforeEach(function () {
    ...
    $this->project = Project::factory()->create(['user_id' => $this->user->id, 'due_date' => '2025-12-31']);
});

it('creates a task successfully', function () {
    $response = $this->postJson(str_replace('{project}', $this->project->id, TASK_ENDPOINT), [
        'name' => TASK_NAME,
...
it('validates task due date is not later than project due date', function () {
    $response = $this->postJson(str_replace('{project}', $this->project->id, TASK_ENDPOINT), [
        'name' => TASK_NAME,
        'description' => TASK_DESCRIPTION,
        'due_date' => '2026-01-01', // Invalid due date
    ], [
        'Accept' => 'application/json',
        'Authorization' => "Bearer {$this->token}",
    ]);
...
it('retrieves a task successfully', function () {
    ...
    $response = $this->getJson(str_replace('{project}', $this->project->id, TASK_ENDPOINT) . '/' . $task->id, [
        'Accept' => 'application/json',
        'Authorization' => "Bearer {$this->token}",
    ]);
...
it('updates a task successfully', function () {
    ...
    $response = $this->putJson(str_replace('{project}', $this->project->id, TASK_ENDPOINT) . '/' . $task->id, [
...
it('soft deletes a task successfully', function () {
    ...
    $response = $this->deleteJson(str_replace('{project}', $this->project->id, TASK_ENDPOINT) . '/' . $task->id, [], [
        'Accept' => 'application/json',
        'Authorization' => "Bearer {$this->token}",
    ]);
